...
if [[ "$repo_name" == "Scripts" ]]; then
    ...
    for dir in "${!formats[@]}"; do
        mkdir -p "Scripts-repo/$dir/Ruleset"
    done
    for key in "${!ruleC_download[@]}"; do
        file="Scripts-repo/Ruleset/${ruleC_download[$key]}"
        base="${ruleC_download[$key]%.*}"
        [[ -f "$file" ]] || { echo "Skipped: $file"; continue; }
        for dir in "${!formats[@]}"; do
            cp "$file" "Scripts-repo/$dir/Ruleset/$base.${formats[$dir]}"
        done
        echo "Processed: $file"
    done
